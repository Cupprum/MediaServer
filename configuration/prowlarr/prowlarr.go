...
func GetConfig() (*Config, error) {
	...
	var err error
	c := &Config{}
	if c.Url, err = utils.RequireEnv("MEDIASERVER_PROWLARR_URL"); err != nil {
		return nil, err
	}
	if c.Username, err = utils.RequireEnv("MEDIASERVER_PROWLARR_USERNAME"); err != nil {
		return nil, err
	}
	if c.Password, err = utils.RequireEnv("MEDIASERVER_PROWLARR_PASSWORD"); err != nil {
		return nil, err
	}
	if c.QBittorrentHostname, err = utils.RequireEnv("MEDIASERVER_QBITTORRENT_HOSTNAME"); err != nil {
		return nil, err
	}
	if c.QBittorrentUsername, err = utils.RequireEnv("MEDIASERVER_QBITTORRENT_USERNAME"); err != nil {
		return nil, err
	}
	if c.QBittorrentPassword, err = utils.RequireEnv("MEDIASERVER_QBITTORRENT_PASSWORD"); err != nil {
		return nil, err
	}
	if c.FlaresolverrHostUrl, err = utils.RequireEnv("MEDIASERVER_FLARESOLVERR_HOST_URL"); err != nil {
		return nil, err
	}
	...
	for _, i := range c.Indexers {
		if i.Enabled {
			if err = c.setIndexer(i); err != nil {
				log.Println("-- error: failed to add indexer:", i.Name)
			}
		}
	}
